...
const createLegacyRef = () => ({current: null});
const METHODS_TO_BIND = [
	'setVideoRef',
	'setPlayerBackButtonRef',
	'handleLoginSuccess',
	'openCreatorPicker',
	'closeCreatorPicker',
	'handleCreatorActivate',
	'handleSearchChange',
	'clearSearch',
	'handleChannelFilter',
	'loadMoreVideos',
...
class AppBase extends Component {
	constructor(props) {
		...
		this.videoRef = createLegacyRef();
		this.videoEventHandlers = [];
		this.focusTimeout = null;
		this.playerFocusTimeout = null;
		this.globalKeyHandler = null;
		this.playerBackButtonNode = null;
		this.currentVideoLoad = null;
		this.playbackRafId = null;
		this.playbackInitiatedToken = null;
		this.playbackStartTimestamp = 0;
		this.watchKeyCache = new Map();
		this.hlsInstance = null;
		this.overlayUpdater = typeof window !== 'undefined' ? window.__updateLegacyOverlay : null;
		if (typeof this.overlayUpdater === 'function') {
			this.overlayUpdater('Legacy app: constructor', false);
		}
	...
	componentDidMount() {
		if (typeof this.overlayUpdater === 'function') {
			this.overlayUpdater('Legacy app: componentDidMount', false);
		}
	...
	componentDidCatch(error) {
		if (typeof this.overlayUpdater === 'function') {
			this.overlayUpdater(`Legacy app error: ${error && error.message ? error.message : error}`, true);
		}
	}
	...
	setPlayerBackButtonRef(node) {
		this.playerBackButtonNode = node;
	}

	hasSpotlightMethod(method) {
		return typeof Spotlight !== 'undefined' && Spotlight && typeof Spotlight[method] === 'function';
	}
	...
	async fetchWatchKey(watchKeyUrl) {
		if (!watchKeyUrl) {
	...
	async initializeCreator(creator, index) {
...
startPlaybackSequence() {
					...
					<div className="creatorHero__header">
						...
						<div className="creatorHero__actions">
							<Button
								size="small"
								icon="list"
								onTap={this.openCreatorPicker}
								data-spotlight-id="switchCreatorButton"
							>
								Switch Creator
							</Button>
							<Button size="small" onTap={() => this.setView('plans')}>
								View Plans
							</Button>
						</div>
					</div>
		...
		return (
			<div className="searchRow">
				<div className="searchInput">
					...
					/>
					{hasSearch ? (
						<Button className="searchClear" size="small" icon="closex" onTap={this.clearSearch}>
							Clear
						</Button>
					) : null}
				</div>
		...
		return (
			<div className={containerClass}>
				<Button
					...
					data-channel-id=""
					onTap={this.handleChannelFilter}
	...
	return (
			<FocusableCard
				...
				role="button"
				onSpotlightSelect={this.handleVideoActivate}
			>
				<div
	...
	renderVideosGrid() {
		...
		if (typeof this.overlayUpdater === 'function') {
			const statusMessage = user
				? showPlayer
					? `Legacy app: showing player (video=${selectedVideo ? selectedVideo.title : 'none'})`
					: `Legacy app: home view (${visibleVideos.length} videos)`
				: 'Legacy app: login view';
			this.overlayUpdater(statusMessage, false);
		}
			...
			<div className="contentSection">
				{showSpinner ? (
					...
					<div className="videoArea">
						<Scroller
							className="videoGridWrapper"
							data-spotlight-container="true"
							data-spotlight-id="videoGridContainer"
							data-spotlight-default-element="videoCard-0"
							data-spotlight-prev-left="sideNav"
							data-spotlight-next-left="sideNav"
							focusableScrollbar
						>
							<div className="videoVirtualGrid">
						...
						{showLoadMore ? (
							<div className="loadMoreRow">
								<Button onTap={this.loadMoreVideos} size="large" disabled={loadingVideos}>
			...
			<div className="contentSection liveSection">
				<div className="liveCards">
					<div className="liveCard liveCard--primary">
						...
						{hasLive ? (
							<div className="liveCard__details">
								...
								<div className="liveActions">
									<Button size="large" icon="play" onTap={this.handlePlayLiveStream}>
										Watch Live
									</Button>
							...
							<div className="liveCard__placeholder">
								<BodyText>No live broadcast is currently active.</BodyText>
								{liveReplay ? (
									<div className="liveCard__replayCallout">
										<BodyText>Catch the latest WAN Show replay below.</BodyText>
										<Button size="large" onTap={() => this.playVideoById(liveReplay.id)}>
											Watch Latest Replay
										</Button>
									</div>
								) : null}
				...
				<div className="liveSection__replays">
					...
					{liveReplay ? (
						<div className="liveReplayCard">
							...
							<Button onTap={() => this.handlePlaySelected()} size="large">
								Play Latest Replay
							</Button>
						</div>
					) : (
			...
			>
				<div className="creatorPicker">
					<div className="creatorPicker__header">
						...
						<Button size="small" icon="closex" onTap={() => this.closeCreatorPicker()}>
							Close
						</Button>
					</div>
					<Scroller className="creatorPicker__list">
						{creators.map((creator, index) => (
							<Item
								...
								onTap={this.handleCreatorActivate}
			...
			<Panel>
				...
				<div className="appPanelBody homePanelBody homePanelBody--twoPane">
					{loadingCreators && !creators.length ? (
						...
						<div className="homePanelColumns">
							<div
							...
							>
								...
								<Button className="sideNav__logout" onTap={this.handleLogout} size="small">
									Log Out
								</Button>
							</div>
				...
				<Header title={(selectedVideo && selectedVideo.title) || 'Now Playing'} subtitle={subtitle} />
					<div className="appPanelBody playerPanelBody">
						<div className="playerContainer">
							...
							>
				<Button
					onTap={this.handleStopPlayback}
					size="large"
					data-spotlight-id="player-back-button"
					componentRef={this.setPlayerBackButtonRef}
				>
									Back to Home
								</Button>
												...
												return (
													<Button
														...
														data-spotlight-id={`quality-${(label || 'auto')
															.toLowerCase()
															.replace(/[^a-z0-9]+/g, '-')}`}
														onTap={this.handleQualityActivate}
														size="small"
													>
														{label}
													</Button>
